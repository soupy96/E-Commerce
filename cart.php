...
if (!empty($_SESSION['cart'])) {
	...
	$q = "SELECT prod_id, CONCAT_WS(' ', cust_fname, cust_lname) AS customers, prod_name FROM customers, products WHERE customers.cust_id = products.cust_id AND products.prod_id IN (";
	foreach ($_SESSION['cart'] as $pid => $value) {
		$q .= $pid . ',';
	}
	$q = substr($q, 0, -1) . ') ORDER BY customers.cust_lname ASC';
